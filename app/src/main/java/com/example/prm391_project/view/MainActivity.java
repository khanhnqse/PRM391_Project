...
public class MainActivity extends BaseActivity {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    if (!list.isEmpty()) {
                        ...
                        binding.bestFoodView.setOnTouchListener((v, event) -> {
    ...
    private Runnable autoScrollRunnable = new Runnable() {
        @Override
        public void run() {
            if (binding.bestFoodView != null && binding.bestFoodView.getAdapter() != null) {
                int totalScrollRange = binding.bestFoodView.computeHorizontalScrollRange();
                int currentScrollOffset = binding.bestFoodView.computeHorizontalScrollOffset();

                if (currentScrollOffset + binding.bestFoodView.getWidth() >= totalScrollRange) {
                    binding.bestFoodView.scrollToPosition(0);
                } else {
                    binding.bestFoodView.smoothScrollBy(scrollOffset, 0);
                }
